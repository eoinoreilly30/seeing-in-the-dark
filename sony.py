...
Original file is located at
    https://colab.research.google.com/drive/1OrL5Ea4nXVSDslkqw-yPd5uc672mSj5r
"""
...
if TESTING:
    input_list = csv.reader(open('/content/drive/My Drive/FYP/sony/test_list_no_burst.txt'), delimiter=" ")
else:
    input_list = csv.reader(open('/content/drive/My Drive/FYP/sony/train_list_no_burst.txt'), delimiter=" ")
    validation_list = csv.reader(open('/content/drive/My Drive/FYP/sony/validation_list_no_burst.txt'), delimiter=" ")
...
# input_fns = input_fns[0:1]
# input_gt_fns = input_gt_fns[0:1]
# validation_fns = validation_fns[0:1]
# validation_gt_fns = validation_gt_fns[0:1]

input_gt_fns = [fn.split('.')[0]+'.ARW' for fn in input_gt_fns]
validation_gt_fns = [fn.split('.')[0]+'.ARW' for fn in validation_gt_fns]
...
if TESTING:
    ...
    input_layer = Input(shape=(512, 512, 4))
...
model.load_weights('/content/drive/My Drive/FYP/sony/weights_16_bit/3873-0.8337.hdf5')

model.compile(Adam(lr=1e-4), loss="mean_absolute_error", metrics=['accuracy'])

checkpoint = ModelCheckpoint('/content/drive/My Drive/FYP/sony/weights_16_bit/{epoch:04d}-{val_acc:.4f}.hdf5', 
                             monitor='val_acc', 
                             verbose=0,
                             save_best_only=True)
...
# today = date.today()
# csv_logger = CSVLogger('/content/drive/My Drive/FYP/sony/logs/training-' + today + '.log')
...
if TESTING:
    ...
    out = model.predict(DataGenerator(input_fns[0:45], input_gt_fns[0:45], batch_size),
                        use_multiprocessing=True,
                        workers=4,
                        verbose=1)
    ...
    for idx, im in enumerate(out):
        ...
        scipy.misc.toimage(im*255, high=255, low=0, cmin=0, cmax=255).save(filename)
    ...
    history = model.fit(DataGenerator(input_fns, input_gt_fns, batch_size),
                        ...
                        workers=4,
                        max_queue_size=10,
                        callbacks=[checkpoint, learning_rate_reduce, csv_logger])
    
    history_file = open('/content/drive/My Drive/FYP/sony/history/' + today + '.pickle', 'wb')
    pickle.dump(history, history_file)
    history_file.close()
    
    total_time = time.time() - start_time
    time_file = open('/content/drive/My Drive/FYP/sony/train_time.txt', 'a')
